...
contract DMTPMessages is IDMTPMessages {
    ...
    function deleteDelegator(address _delegator) external override {
        delegators[msg.sender][_delegator] = false;
        emit DelgetorDelete(msg.sender, _delegator);
    }

    function isDelegator(address _owner, address _delegator)
        public
        view
        returns (bool)
    {
    ...
    function sendMessages(
        ...
        require(
            delegators[_from][msg.sender] == true || msg.sender == _from,
            "DMTPMessages : Invalid delegators"
        );
